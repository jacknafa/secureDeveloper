...
>main.go:142
>main.go:429
>SQL Injection을 막기 위해 Prepared Statement를 적용해 방어

지금까지 구현한 기능은 회원가입, 게시글 작성 및 조회 기능 뿐이다.

> 이슈
내 정보에서 사용자 정보가 원래는 로그인 전에는 뜨지 않았다가 로그인 후에 나타나고 다시 로그아웃을 하면 정보가 뜨지 않아야하는데 로그인 후와 로그아웃 후에 계속 이전 사용자의 정보가 뜨는 상황이 있었다.
원래는 logout과 withdraw API에서 sessions.delete(token) 해당 코드를 통해 토큰을 지워야 하는게 맞지만 혹시 몰라 내 정보를 불러오는 API에도 코드를 추가하고 서버를 실행하니 
if _, ok := sessions.lookup(token); !ok {
				c.JSON(http.StatusUnauthorized, gin.H{"message": "invalid authorization token"})
				return
			}

해당 코드가 실행하고 오류가 뜨면서 기능이 작동하질 않았다. 

알아보니 FE 측에서 쿠키 관련한 오류(?) 가 발생한 것으로 그냥 로그아웃하고 페이지를 새로고침하면 의도한대로 사용자 정보가 뜨질 않았다.
